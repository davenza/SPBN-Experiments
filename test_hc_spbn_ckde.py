...
def test_validation_spbn(train_data, test_data, folds, patience, result_folder, idx_fold):
    ...
    for idx_k, k in enumerate(folds):
        for idx_p, p in enumerate(patience):
            models_folder = result_folder + '/HillClimbing/SPBN_CKDE/Validation_' + str(k) + "_" + str(p) + '/' + str(idx_fold)
...
def test_crossvalidation():
    ...
    string_file = "Dataset," + ','.join(["SPBN_CKDE_Validation_" + str(f) + "_" + str(p) for f in folds for p in patience])
    ...
    with open('results_hc_spbn_ckde.csv', 'w') as f:
        f.write(string_file)
