...
def run_hc_spbn(d, idx_dataset, i):
    ...
    for p in experiments_helper.PATIENCE:
        result_folder = 'models/' + d + '/' + str(idx_dataset).zfill(3) + '/' +  str(i) + '/HillClimbing/SPBN/' + str(p)
    ...
    for i in experiments_helper.INSTANCES:
        for idx_dataset in range(0, math.ceil(experiments_helper.NUM_SIMULATIONS / 10)):
            ...
            with mp.Pool(processes=num_processes) as p:
                p.starmap(run_hc_spbn, [(d, 10*idx_dataset + ii, i) for ii in range(num_processes)]
                            )
