...
def compare_models(true_model, dataset_name, num_instances):
    ...
    for p in experiments_helper.PATIENCE:
        for i in range(experiments_helper.NUM_SIMULATIONS):
            ...
            spbn_ckde_folder = 'models/' + dataset_name + '/' + str(i).zfill(3) + '/' + str(num_instances) + '/HillClimbing/SPBN_CKDE/' + str(p)
            all_models = sorted(glob.glob(spbn_ckde_folder + '/*.pickle'))
        ...
        print("Mean SPBN CKDE patience " + str(p) + " results: ")
