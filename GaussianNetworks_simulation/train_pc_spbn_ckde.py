    ...
    for i in experiments_helper.INSTANCES:
        for idx_dataset in range(experiments_helper.NUM_SIMULATIONS):
            ...
            for p in experiments_helper.PATIENCE:
                result_folder = 'models/' + d + '/' + str(idx_dataset).zfill(3) + '/' + str(i) + '/PC/SPBN_CKDE/LinearCorrelation/' + str(p)
                ...
                if not os.path.exists(result_folder + '/end.lock'):
                    ...
                    bn = hc.estimate(change_node, vl, start_model, callback=cb_save, patience=p, verbose=True)
            ...
            for p in experiments_helper.PATIENCE:
                result_folder = 'models/' + d + '/' + str(idx_dataset).zfill(3) + '/' + str(i) + '/PC/SPBN_CKDE/RCoT/' + str(p)
                pathlib.Path(result_folder).mkdir(parents=True, exist_ok=True)
