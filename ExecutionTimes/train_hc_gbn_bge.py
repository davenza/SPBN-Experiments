...
from pybnesian.learning.scores import BGe
...
for n in experiments_helper.INSTANCES:
    ...
    for i in range(20000):
        ...
        start = time.time()
        bn = hc.estimate(arcs, bge, start_model)
        end = time.time()

        executions[i] = end - start

    small_results['GBN_BGe_' + str(n)] = pd.Series(executions, name="GBN_BGe_" + str(n))
    ...
    medium_results['GBN_BGe_' + str(n)] = pd.Series(executions, name="GBN_BGe_" + str(n))
    ...
    large_results['GBN_BGe_' + str(n)] = pd.Series(executions, name="GBN_BGe_" + str(n))
    print("Large " + str(n) + " -- Time: " + str(executions.mean()) + ", std: " + str(np.std(executions, ddof=1)))

small_results.to_csv("HC_GBN_BGe_small.csv", index=False)
medium_results.to_csv("HC_GBN_BGe_medium.csv", index=False)
large_results.to_csv("HC_GBN_BGe_large.csv", index=False)